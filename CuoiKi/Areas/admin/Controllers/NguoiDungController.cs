    ...
    public class NguoiDungController : Controller
    {
        ...
        public ActionResult Edit(string id)
        {
            ...
            foreach (XmlElement ele in xmlDoc.GetElementsByTagName("NguoiDung"))
            {
                if (ele.GetAttribute("id").ToString() == id)
                {
                    ...
                    model.gioiTinh = ele.GetAttribute("gioiTinh");
                    if(ele.GetAttribute("trangThai").ToString() == "1")
                    {
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (XmlElement ele in xmlDoc.GetElementsByTagName("NguoiDung"))
                {
                    if (ele.GetAttribute("id").ToString() == nguoiDung.id)
                    {
                        ...
                        ele.SetAttribute("matKhau", Encryptor.EncryptMD5(nguoiDung.matKhau));
                        ...
                        ele.SetAttribute("gioiTinh", nguoiDung.gioiTinh);
                        ...
                        xmlDoc.Save(Server.MapPath("~/App_Data/NguoiDung.xml"));
                        UpdateToSQL(nguoiDung.id);//cập nhật dòng tài khoản có id là mã TK của nhân viên
                        break;
                    }
                }
            ...
            foreach (DataRow dataRow in dt.Rows)
            {
                if (dataRow["id"].ToString() == id)
                {
                    sql = "update NguoiDung set ten = N'" + dataRow["ten"] + "', diaChi = N'" + dataRow["diaChi"] + "', SDT = '" + dataRow["SDT"] + "', email = '" + dataRow["email"] + "', " +
                        "CMND = '" + dataRow["CMND"] + "', matKhau = '" + dataRow["matKhau"] + "', maQuyen = " + dataRow["maQuyen"] + ", trangThai = " + dataRow["trangThai"] + ", gioiTinh = " + dataRow["gioiTinh"] + "where id = " + id;
        ...
        public void ToNguoiDungSQL()
        {
            ...
            foreach (DataRow dataRow in dt.Rows)
            {
                ...
                {
                    sql = "insert into NguoiDung(tenTaiKhoan, matKhau, maQuyen, ten, diaChi, SDT,  email, CMND, trangThai, gioiTinh)" +
                        ...
                        "', " + dataRow["trangThai"] + ", " + dataRow["gioiTinh"] + ")";
                    XmlToSQL.InsertOrUpDateSQL(sql);
                }
            }
        }

        //public void ConvertTaiKhoanToXml()
        //{
        ...
        public JsonResult TimKiemAjax(string keyword)
        {
            ...
            if (keyword != null)
            {
                foreach (NguoiDung nd in list)
                {
                    if (nd.ten.ToLower().Contains(keyword.ToLower()))
                    {
                        ListFind.Add(nd);
                    }
                }
            ...
            {
                list = ListFind,
                message = ListFind.Count() > 0 ? "" : "Không tìm thấy người dùng nào!"
            });
        }
    }
}